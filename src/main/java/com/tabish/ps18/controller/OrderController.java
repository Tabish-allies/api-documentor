...
public class OrderController {
    ...
    @Operation(
            ...
            description = "Places a new order. Business Rules: Product must exist and be available. Requested quantity must not exceed available stock. Stock is automatically deducted upon order creation. Total amount is calculated as price x quantity. Initial status is always PENDING.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    content = @Content(examples = @ExampleObject(
                            name = "Create Order",
                            value = "{\"customerEmail\": \"john.doe@example.com\", \"customerName\": \"John Doe\", \"product\": {\"id\": 1}, \"quantity\": 2}"
                    ))
            )
    )
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "Updates the status of an order. Business Rules: Valid transitions: PENDING -> CONFIRMED -> SHIPPED -> DELIVERED. CANCELLED can be set from PENDING or CONFIRMED only. Status cannot move backwards (except to CANCELLED).",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    content = @Content(examples = @ExampleObject(
                            name = "Confirm Order",
    ...
    @Operation(
            ...
            description = "Cancels an order and restores inventory. Business Rules: Only PENDING and CONFIRMED orders can be cancelled. Stock is automatically restored to the product inventory. Cannot cancel SHIPPED or DELIVERED orders."
    )
    @ApiResponses({
